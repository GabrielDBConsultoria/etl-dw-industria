...
def load_fato_vendasitens():
    ...
    SELECT
        ...
        mov.mvd_mtdcodigo AS ipv_mtdcodigo,
    ...
    for chunk in pd.read_sql(query, conn_erp, chunksize=5000):
        ...
        for _, row in chunk.iterrows():
            nat = row.ped_natcodigo
            mtdcode = row.ipv_mtdcodigo

            if nat in ['DEV', 'DTR', 'DV']:
                if pd.isna(mtdcode) or mtdcode not in valid_mot_codes:
                    mtdcode = 999999
            else:
                mtdcode = 999999
